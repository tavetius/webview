...
import { addOnboadingData, stopServer } from '../../utils/api';
...
const Onboarding = ({
  ...
  const analysisHandler = () => {
    ...
    if (!userUploads.hasOwnProperty('noExistingFilesErrorMessage')) {
      deleteFiles([], Object.keys(userUploads), userFullInfo).then(() => {
        console.log("delete")
      })

    }
    if (servers && Object.keys(servers).length === 1) {
      stopServer(Object.keys(servers)[0]).then((response) => {
        if (response.error) {
            ...
            return (
              <div key={indx}>
                <button
                  ...
                  className={`onboarding-btn ${indx === descrChoice ? 'active-choice' : ''
                    }`}
                >
                  {desc}
                </button>
