      ...
      <div className="select-box--box" ref={this.wrapperRef}>
        <div className="select-box--container d-flex">
          ...
          >
              ...
              coreToKNode(
                null,
                items.length<=coreIndex[engineName]?items[0]:items[coreIndex[engineName]],
                engineName
              )
              : items[0]}
          </div>
            ...
            {type === 'cores'
              ? items.map((item, index) => (
                <div
                  key={index}
                  onClick={(e) => {
                    if (!disabledEngineCore(userFullInfo, item)) {
                      e.preventDefault();
                      this.props.setOpenCoinsModal(true);
                    } else {
                      this.selectItem(e, item, engineName);
                    }
                  }}
                  className={
                    disabledEngineCore(userFullInfo, item)
                      ? ''
                      : 'disabled-item'
                  }
